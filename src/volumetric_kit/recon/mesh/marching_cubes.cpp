...
// tier's spatial-hash primes (volume/hash.hpp) so the mixing matches the rest
// of the codebase; the map is a plain host container, not the device table.
struct CoordHash {
  std::size_t operator()(const Vec3i& c) const noexcept {
    return (static_cast<std::uint32_t>(c.x) * 73856093u) ^
           (static_cast<std::uint32_t>(c.y) * 19349669u) ^
           (static_cast<std::uint32_t>(c.z) * 83492791u);
  }
};
...
Result<Mesh> MarchingCubes::extract(volume::VoxelBlockGrid& grid, float iso) {
  ...
  kernel_sparse_.set.write_storage_buffer(
      ...
      VK_WHOLE_SIZE);
  kernel_sparse_.set.write_storage_buffer(6, vertices_buf.handle(), 0,
                                          VK_WHOLE_SIZE);
  kernel_sparse_.set.write_storage_buffer(7, counter_buf.handle(), 0,
                                          VK_WHOLE_SIZE);
  ...
  std::uint32_t emitted = 0;
  std::memcpy(&emitted, counter_buf.mapped(), sizeof(std::uint32_t));
  emitted = std::min(emitted, capacity);  // defensive clamp (see dense extract)
