...
Result<std::uint32_t> VoxelHashMap::allocate_from_triangles(
  ...
  if (vertices == nullptr || indices == nullptr) {
    return Status::invalid_argument(
        "VoxelHashMap::allocate_from_triangles: vertices or indices is null");
  }

  // The host pass: one candidate-block count per triangle, as an exclusive
  // prefix sum. This is what buys the kernel a uniform work item (see the
  // shader header) -- and it is also the only place an index can be bounds-
  // checked, since the kernel indexes `vertices` with it directly and
  // robustBufferAccess is enabled nowhere in this repo.
  std::vector<std::uint32_t> offsets(std::size_t{triangle_count} + 1);
  ...
  for (std::uint32_t t = 0; t < triangle_count; ++t) {
    ...
    if (i0 >= vertex_count || i1 >= vertex_count || i2 >= vertex_count) {
      return Status::invalid_argument(
          "VoxelHashMap::allocate_from_triangles: a triangle index is at or "
          "past vertex_count");
    }
    ...
    // Mirrored bit-for-bit by the kernel, which recomputes rather than reads
    // this back -- see the shader.
    const Vec3f lo = glm::min(v0, glm::min(v1, v2)) - Vec3f(grid_.trunc_dist);
    const Vec3f hi = glm::max(v0, glm::max(v1, v2)) + Vec3f(grid_.trunc_dist);
    const Vec3i bmin = world_to_block(lo, grid_);
    const Vec3i bmax = world_to_block(hi, grid_);
    ...
    // saying so beats truncating the count and allocating a fraction of it.
    if (total > std::numeric_limits<std::uint32_t>::max()) {
      return Status::invalid_argument(
          "VoxelHashMap::allocate_from_triangles: candidate blocks exceed 2^32 "
          "-- is the mesh scaled to the grid's units (metres)?");
    }
  }
  ...
  const auto work_items = static_cast<std::uint32_t>(total);

  // Three transient inputs (the mesh is per-call and variable), each bound
  // whole, so each is checked against what one binding may cover.
  ...
  return dispatch_with_retry(triangles_, triangle_count,
                             group_count(work_items), out_failures, &stage);
}
