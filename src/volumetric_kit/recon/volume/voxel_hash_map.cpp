...
Result<std::uint32_t> VoxelHashMap::run_coord_kernel(
  ...
  // Re-dispatch while the failure count keeps dropping: allocations/removals
  // can spuriously fail under heavy same-bucket contention (a GPU spin-lock
  // livelock within a SIMD group). Already-processed coords take the lock-free
  // fast path on the next round (allocate) or become a no-op (remove -- the
  // delete kernel skips a coord it no longer finds without counting a failure),
  // so contention falls and the set converges -- unless a genuine capacity
  // limit (chain full / heap empty) stalls progress. Mirrors the prior engine's
  // launchWithRetry.
  //
  ...
  // allocate and delete never run in the same dispatch. Re-zeroed each round.
  ...
  for (int round = 0; round < kMaxRounds; ++round) {
    ...
    VR_TRY(
        dispatch(*device_, pipeline, set.handle(), push, group_count(count)));
